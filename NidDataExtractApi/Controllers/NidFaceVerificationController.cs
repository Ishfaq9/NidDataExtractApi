    ...
    public class NidFaceVerificationController : ControllerBase
    {
        ...
        private async Task<Response> faceVerifyResponse(string imageBase641, string imageBase642)
        {
            try
            {
                ...
                var response = await client.PostAsync("http://localhost:6000/compare-faces/", content);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (imageResult.threshold == 0)
                        return await faceVerifyResponseDeepFace(imageBase641, imageBase642);
        ...
        private async Task<Response> faceVerifyResponseDeepFace(string imageBase641, string imageBase642)
        {
            try
            {
                var content = new StringContent(
                ...
                var response = await client.PostAsync("http://localhost:6001/compare-faces/", content);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    imageResult.distance = 1- imageResult.distance;
